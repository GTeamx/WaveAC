...
public class ActionData implements Data {
    ...
    public void process(final ServerPlayPacket serverPlayPacket) {
        ...
        if (serverPlayPacket.is(PacketType.Play.Server.BLOCK_ACTION)) {

            final WrapperPlayServerBlockAction blockActionWrapper = serverPlayPacket.getBlockActionWrapper();

            TaskUtils.task(() -> Bukkit.broadcastMessage("adata=" + blockActionWrapper.getActionData()));
        }
    }
