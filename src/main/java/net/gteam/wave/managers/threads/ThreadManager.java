package net.gteam.wave.managers.threads;
...
import net.gteam.wave.utils.custom.exception.WaveException;
...
public class ThreadManager implements Listener, Initializer {
    ...
    public ThreadManager(final Wave plugin) {
        this.plugin = plugin;
    }

    @Override
    public void initialize() {
        Bukkit.getPluginManager().registerEvents(this, Wave.getInstance());
    }
    ...
    public ProfileThread getAvailableProfileThread() {
        ...
        if (profileThread == null) {
            throw new WaveException("Encountered a null profile thread, Please restart the server to avoid any issues.");
        }
